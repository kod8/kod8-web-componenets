const havaDurumuTemplate = document.createElement("template");
havaDurumuTemplate.innerHTML = `
...
class havaDurumu extends HTMLElement {
  constructor() {
    super();

    this.attachShadow({ mode: "open" });
    this.shadowRoot.appendChild(havaDurumuTemplate.content.cloneNode(true));
  ...
  connectedCallback() {
    this.renderCity(this.getAttribute("city"))
    this.fetchData();
...
fetchData = function () {
  ...
  renderCity = function (plate){
    ...
    this.otherDaysElement.forEach((dayElement,index)=>{
      ...
      var iconId = this.conditions[data[index].condition].icon
      dayElement.querySelector(".icon").innerHTML = this.weatherIcons[iconId]["day"];
      //dayElement.querySelector(".condition").innerText = data[index].condition;
      dayElement.querySelector(".maxDegree").innerText = data[index].max
      dayElement.querySelector(".minDegree").innerText = data[index].min
    })
  ...
  handleSelectInput = function(e){
    if(this.getAttribute("city")!==e.target.value) this.setAttribute("city", e.target.value);
  }
...
window.customElements.define("hava-durumu", havaDurumu)
